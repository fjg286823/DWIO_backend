...
void ITMSceneReconstructionEngine_CUDA<TVoxel>::AllocateSceneFromDepth(ITMScene<TVoxel, ITMVoxelBlockHash> *scene,
    ...
    buildHashAllocAndVisibleType_device <<<gridSizeHV, cudaBlockSizeHV >>>
            (entriesAllocType_device, entriesVisibleType, blockCoords_device, depth_map,
...
void ITMSceneReconstructionEngine_CUDA<TVoxel>::AllocateScene(ITMScene<TVoxel, ITMVoxelBlockHash> *scene,
    ...
    buildCsmAllocAndVisibleType_device<<<gridSizeCSM, cudaBlockSizeCsm >>>
            (entriesAllocType_device, entriesVisibleType, blockCoords_device, blockSize, hashTable,
    ...
    buildCsmVisibleList_device<true> <<<gridSizeAL, cudaBlockSizeAL >>>
            (hashTable, swapStates, noTotalEntries, visibleEntryIDs, (AllocationTempData *) allocationTempData_device,
...
void ITMSceneReconstructionEngine_CUDA<TVoxel>::SwapAllBlocks(ITMScene<TVoxel, ITMVoxelBlockHash> *scene,
    ...
    findNeedtoSwapInBlocks_device<<<gridSizeAL, cudaBlockSizeAL>>>
            (hashTable, noTotalEntries, entriesVisibleType, States, swapStates, NeedToSwapIn_device);
...
void ITMSceneReconstructionEngine_CUDA<TVoxel>::IntegrateIntoScene(ITMScene<TVoxel, ITMVoxelBlockHash> *scene,
    ...
    projParams_rgb = camera_intrinsic;
    ...
    integrateIntoScene_device<TVoxel, true> <<<gridSize, cudaBlockSize >>>
            (localVBA, hashTable, visibleEntryIDs, depth_map, rgb,
...
namespace {
    ...
    __global__ void integrateIntoScene_device(TVoxel *localVBA,
        ...
        if (localVoxelBlock[locId].w_depth == maxW) return;//这里可能是个问题
        ...
        ComputeUpdatedVoxelInfo<true, false, TVoxel>::compute
                (localVoxelBlock[locId], pt_model, M_d, projParams_d, M_rgb, projParams_rgb, mu, maxW, depth_map, color_map);
    }
    ...
    __global__ void buildHashAllocAndVisibleType_device(uchar *entriesAllocType,
        ...
        if (x > depth.cols - 1 || y > depth.rows - 1) return;
    ...
    __global__ void buildVisibleList_device(ITMHashEntry *hashTable,
        ...
        if (shouldPrefix) {
            int offset = computePrefixSum_device<int>
                    (hashVisibleType > 0, &allocData->noVisibleEntries, blockDim.x * blockDim.y, threadIdx.x);
