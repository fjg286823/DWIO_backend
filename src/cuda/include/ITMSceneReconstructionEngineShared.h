                                                     ...
                                                     const PtrStepSz<float> depth)
{
    ...
    // 如果点的深度大于相机深度阈值就不更新了，这样确实可以减少一下更新
    if ((pt_image.x() < 1) || (pt_image.x() > depth.cols - 2) || (pt_image.y() < 1) || (pt_image.y() > depth.rows - 2))
        return -1;
    ...
    if (abs(eta) > mu) // eta>=-mu
        return eta;
    oldF = TVoxel::valueToFloat(voxel.tsdf);
    oldW = voxel.w_depth;
    newF = fmin(1.0f, eta / mu);
    newW = 1;
...
template <class TVoxel>
__device__ inline void computeUpdatedVoxelDepthAndRgbInfo(DEVICEPTR(TVoxel) & voxel,
                                                          const Vector4f &pt_model,
                                                          const Eigen::Matrix4f &M_d,
                                                          const Vector4f &projParams_d,
                                                          float mu,
                                                          ...
                                                          const PtrStepSz<uchar3> rgb)
{
    Vector4f pt_camera;
    Vector2f pt_image;
    float depth_measure, sdf, oldF, newF;
    int oldW, newW, addW;
    ...
    sdf = (-1.f) * ((1.f / lambda) * pt_camera.norm() - depth_measure);

    if (sdf >= -mu)
    {
        oldF = TVoxel::valueToFloat(voxel.tsdf);
        oldW = voxel.w_depth;
        newF = fmin(1.0f, sdf / mu);
        newW = 1;
        addW = 1;
        ...
        oldC = voxel.clr;
        // float3 res;
        // if(voxel.w_depth==0){//确保第一次的颜色没有问题
        //     voxel.tsdf = TVoxel::floatToValue(newF);
        //     voxel.w_depth = static_cast<short>(newW);
        // 	   voxel.clr =  newC;
        // }
        ...
        voxel.clr.x = static_cast<uchar>((oldW * oldC.x + addW * newC.x) / (oldW + addW));
        voxel.clr.y = static_cast<uchar>((oldW * oldC.y + addW * newC.y) / (oldW + addW));
        voxel.clr.z = static_cast<uchar>((oldW * oldC.z + addW * newC.z) / (oldW + addW));
    }
}
                                                    ...
                                                    const PtrStepSz<uchar3> rgb)
{
    ...
    if ((pt_image.x() < 1) || (pt_image.x() > rgb.cols - 2) || (pt_image.y() < 1) || (pt_image.y() > rgb.rows - 2))
        return;
                                                      ...
                                                      uchar *emptyBlockEntries)
{
    ...
    Vector4f pt_buff;
    pt_buff = pt_camera_f * (1.0f - 2*mu / norm);
    pt_buff(3, 0) = 1.0f;
                                            ...
                                            const CONSTPTR(Vector2i) & imgSize)
{
    ...
    else if (useSwapping)
    {
        ...
        if (pt_buff(0, 0) >= lims(0, 0) && pt_buff(0, 0) < lims(1, 0) && pt_buff(1, 0) >= lims(2, 0) && pt_buff(1, 0) < lims(3, 0))
            isVisibleEnlarged = true;
    }
}
