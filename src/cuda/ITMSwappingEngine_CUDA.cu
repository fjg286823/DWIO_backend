                                                              ...
                                                              bool updateFlag)
{
    ...
    if (noNeededEntries > 0)
    {
        ...
        if (!updateFlag)
        {
            integrateOldIntoActiveData_device<<<gridSize, blockSize>>>(localVBA, swapStates, States, syncedVoxelBlocks_local, moveInEntryIDs_local, hashTable, maxW);
            cudaCheckError();
            cudaDeviceSynchronize();
        }
        else
        {
            integrateDataCpuAndDevice<<<gridSize, blockSize>>>(localVBA, swapStates, States, syncedVoxelBlocks_local, moveInEntryIDs_local, hashTable, maxW);
            cudaCheckError();
            cudaDeviceSynchronize();
        }
    }
    ...
    if (moveInEntries > 0)
    {
        ...
        SetBlockSwapstate<<<gridSizeState, cudaBlockSizeState>>>(swapStates, moveInEntries, neededEntryIDs_local);
        cudaCheckError();
        cudaDeviceSynchronize();
    }
    if (noNeededEntries_orgin > SDF_TRANSFER_BLOCK_NUM)
...
void ITMSwappingEngine_CUDA<TVoxel>::TransferGlobalMap(ITMScene<TVoxel, ITMVoxelBlockHash> *scene)
{
    ...
    if (noNeededEntries > 0)
    {
        ...
        SetBlockSwapstate<<<gridSizeState, cudaBlockSizeState>>>(swapStates, noNeededEntries, neededEntryIDs_local);
        cudaDeviceSynchronize();
    }
}
...
void ITMSwappingEngine_CUDA<TVoxel>::SaveToGlobalMemory(ITMScene<TVoxel, ITMVoxelBlockHash> *scene, ITMRenderState_VH *renderState)
{
    ...
    DWIOcudaSafeCall(cudaMemset(noNeededEntries_device, 0, sizeof(int)));
    // 不可见的在gpu上的block索引会被统计起来
    buildRealListToSwapOut_device<<<gridSize, blockSize>>>(neededEntryIDs_local, noNeededEntries_device, swapStates,
    ...
    { // 这里不保证会全部转出去
        noNeededEntries = MIN(noNeededEntries, SDF_TRANSFER_BLOCK_NUM);
        {
            ...
            moveActiveDataToTransferBuffer_device<<<gridSize, blockSize>>>(syncedVoxelBlocks_local, hasSyncedData_local, neededEntryIDs_local, hashTable, localVBA);
            cudaDeviceSynchronize();
        }
    ...
    if (noNeededEntries > 0)
    {
        ...
        cleanMemory_device<<<gridSize, blockSize>>>(voxelAllocationList, noAllocatedVoxelEntries_device, swapStates, hashTable,
...
namespace
{
                                                  ...
                                                  int *sum)
    {
        ...
        bool isNeededId = swapState.state == 2 && hashTable[targetIdx].ptr >= 0 && entriesVisibleType[targetIdx] == 0 && emptyBlockEntries[targetIdx] == 1;
                                                          ...
                                                          TVoxel *localVBA)
    {
        ...
        if (vIdx == 0)
            hasSyncedData_local[blockIdx.x] = true;
    }
    ...
    __global__ void integrateDataCpuAndDevice(TVoxel *localVBA,
                                              ...
                                              int maxW)
    {
        int entryDestId = neededEntryIDs_local[blockIdx.x]; // 这个要融合的block是怎么计算的
