...
//生成一个[径向*角度] （20*60）的二维数组，里面存放着高度值！
...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc ); //得到每一个ring1的高度均值
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc ); //得到每一个角度划分的均值
    std::vector<float> polarcontext_invkey_vec = eig2stdvec( ringkey );
